...
static long altera_dma_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) != ALTERA_IOC_MAGIC)
        return -ENOTTY;
    ...
    switch (cmd) {
    case STP_IOCX_RESET:
        printk (KERN_DEBUG "stp reset\n");
        iowrite32(0xFF, bk_ptr->bar[0]+DESC_CTRLLER_BASE + ALTERA_LITE_DMA_WR_LAST_PTR);
        ret = 0;
        break;
    case STP_IOCX_WRITE_REG:
        ret = copy_from_user(&stp_cmd, (struct stp_cmd *) arg, sizeof(struct stp_cmd)); 
        if (ret)
            break;

//        printk (KERN_DEBUG "write: offset = %x, value = %x\n",  stp_cmd.offset << 2, stp_cmd.value);

        if (stp_cmd.offset == DATA_OUT_2) 
            bk_ptr->stp_p0_base = stp_cmd.value;
        else if (stp_cmd.offset == DATA_OUT_3)
            bk_ptr->stp_p1_base = stp_cmd.value;
        else if (stp_cmd.offset == DATA_OUT_6)
            bk_ptr->stp_buffer_size = stp_cmd.value;

        // We have two register files.
        // The first one starts at Bar[4], offset 0
        // The second one starts at Bar[4], offset 0x800
        reg_bank = stp_cmd.offset / 0x100;
        offset = stp_cmd.offset % 0x100;

        iowrite32(stp_cmd.value,
                  (u32 *)(bk_ptr->bar[4] + 0x800 * reg_bank +
                          (offset << 2)));
        ret = 0;
        break;
    case STP_IOCX_READ_REG:
        ret = copy_from_user(&stp_cmd, (struct stp_cmd *) arg, sizeof(struct stp_cmd)); 
        if (ret)
            break;
        ...
        stp_cmd.value = ioread32((u32 *)(bk_ptr->bar[4] + 0x800 * reg_bank +
                                         (offset << 2)));

        printk (KERN_DEBUG "read: offset = %x, value = %x\n",  stp_cmd.offset << 2, stp_cmd.value);

        ret = copy_to_user((struct stp_cmd *) arg, &stp_cmd, sizeof(struct stp_cmd)); 
        break;
    default:
        printk (KERN_DEBUG "unknown commands");
        ret = -EINVAL;
        break;
    }
	...
	case ALTERA_IOCX_START:
		printk (KERN_DEBUG "altera dma ioctrl");
...
ssize_t stp_dma_read(struct altera_pcie_dma_bookkeep *bk_ptr, char __user *buf, size_t count, loff_t *pos)
{
	...
	memset(rp_wr_buffer_virt_addr, 0, count);

//	printk(KERN_DEBUG "perform DMA read count = %lx, pos = %llx", count, *pos);
	...
	tid = last_id == 0xFF? 0 : (last_id + 1 ) % 128;
//	printk(KERN_DEBUG "last_id = %d, tid = %d", last_id, tid);

//    printk(KERN_DEBUG "P0:%x~%x (%x~%x), P1:%x~%x (%x~%x)\n", OFFCHIP_MEM_BASE + *pos, OFFCHIP_MEM_BASE +*pos + count / 2, rp_wr_buffer_bus_addr, rp_wr_buffer_bus_addr + count / 2,
//                    OFFCHIP_MEM_BASE + *pos + bk_ptr->stp_p1_base, OFFCHIP_MEM_BASE +*pos + bk_ptr->stp_p1_base + count / 2, rp_wr_buffer_bus_addr + (count / 2), rp_wr_buffer_bus_addr + count );

	set_write_desc (&bk_ptr->lite_table_wr_cpu_virt_addr->descriptors[tid], OFFCHIP_MEM_BASE + *pos , (dma_addr_t) rp_wr_buffer_bus_addr , count_dw  / 2, tid);

	set_write_desc (&bk_ptr->lite_table_wr_cpu_virt_addr->descriptors[tid+1], OFFCHIP_MEM_BASE + *pos +bk_ptr->stp_p1_base, (dma_addr_t) rp_wr_buffer_bus_addr + (count / 2), count_dw / 2, tid+1);
	...
	// start DMA
	iowrite32 (tid +1, bk_ptr->bar[0]+DESC_CTRLLER_BASE+ALTERA_LITE_DMA_WR_LAST_PTR);

	while (1) {
		if (bk_ptr->lite_table_wr_cpu_virt_addr->header.flags[tid +1]) {
			break;
		}
...
//	printk(KERN_DEBUG "READ DMA DONE %ld (before copy_to_user)\n", count);

	// copy to user
	if(__copy_to_user(buf, bk_ptr->rp_wr_buffer_virt_addr, count))
		return -EFAULT;

	*pos = (*pos + (count  / 2)) % (bk_ptr->stp_buffer_size);

//	printk(KERN_DEBUG "READ DMA DONE %ld %x\n", count, *pos);
...
out:
	...
	for ( i = 0 ; i < 128 ; i ++ ) {
		if(bk_ptr->lite_table_wr_cpu_virt_addr->header.flags[i]) {
			printk(KERN_DEBUG "%d is not zero\n", i);
		}
	}
	return -EFAULT;
}

ssize_t stp_read(struct file *file, char __user *buf, size_t count, loff_t *pos) 
{
	struct altera_pcie_dma_bookkeep *bk_ptr = file->private_data;
	return stp_dma_read(bk_ptr, buf, count, pos);
}

ssize_t altera_dma_read(struct file *file, char __user *buf, size_t count, loff_t *pos) {
	return stp_read(file, buf, count, pos);
#if 0
...
ssize_t altera_dma_exec_cmd(struct dma_cmd *ucmd, struct altera_pcie_dma_bookkeep * bk_ptr) {
	...
	case ALTERA_CMD_READ_STATUS:
		...
		break;
	case STP_WRITE_REG:
		if (kcmd.usr_buf_size != 5) {
			printk(KERN_DEBUG "invalid command size %d", kcmd.usr_buf_size);
			break;
		}
		stp_reg_offset = (unsigned char) kcmd.buf[0];
        value = *(u32*) (&kcmd.buf[1]);
//		value = kcmd.buf[4] << 24 | kcmd.buf[3] << 16 | kcmd.buf[2] << 8 | kcmd.buf[1];

		printk(KERN_DEBUG "%x %x", stp_reg_offset << 2, value);
        break;

		iowrite32(value, (u32 *)(bk_ptr->bar[4] + (stp_reg_offset << 2)));
		break;
	...
	case STP_READ_REG:
		offset = kcmd.buf[0];
		reg_data = ioread32((u32 *)(bk_ptr->bar[4] + (offset << 2)));
		printk(KERN_DEBUG "offset = %x %x", offset, reg_data);
		break;
...
//	printk(KERN_DEBUG "SRC_ADDR_LDW: 0x%08x", wr_desc->src_addr_ldw);
//	printk(KERN_DEBUG "SRC_ADDR_UDW: 0x%08x", wr_desc->src_addr_udw);
//	printk(KERN_DEBUG "DEST_ADDR_LDW: 0x%08x", wr_desc->dest_addr_ldw);
//	printk(KERN_DEBUG "DEST_ADDR_UDW: 0x%08x", wr_desc->dest_addr_udw);
//	printk(KERN_DEBUG "CTRL_DMA_LEN: 0x%08x", wr_desc->ctl_dma_len);
	return 0;
}
...
static int dma_test(struct altera_pcie_dma_bookkeep *bk_ptr, struct pci_dev *dev)
{
	...
	printk(KERN_DEBUG "set rdbuffer virt address");
	...
	if(bk_ptr->dma_status.run_read) {
		...
		printk (KERN_DEBUG "dma_desc_num: %d", bk_ptr->dma_status.altera_dma_descriptor_num);
		printk (KERN_DEBUG "last_id: %d", last_id);

		if(last_id > 127){
			last_id = last_id - 128;
			if((bk_ptr->dma_status.altera_dma_descriptor_num > 1) && (last_id != 127)) write_127 = 1;
			printk (KERN_DEBUG "write_127: %d", write_127);
		}
	...
	if (bk_ptr->dma_status.run_write) {
		printk(KERN_DEBUG "perform DMA write test");
		timeout = TIMEOUT;
		write_127 = 0;
		last_id = ioread32((u32 *)(bk_ptr->bar[0]+DESC_CTRLLER_BASE+ALTERA_LITE_DMA_WR_LAST_PTR));
	...
	if(bk_ptr->dma_status.run_simul) {
		printk(KERN_DEBUG "perform DMA simul test");
		timeout = TIMEOUT;
		r_write_127 = 0;
		w_write_127 = 0;
...
static int __init altera_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	printk(KERN_INFO "Rootport DMA RD buffer = 0x%08llx", bk_ptr->rp_rd_buffer_bus_addr);
	...
	printk(KERN_INFO "Rootport DMA WR buffer = 0x%08llx", bk_ptr->rp_wr_buffer_bus_addr);
    ...
    // default STP values
    bk_ptr->stp_buffer_size = 0x40000;
    bk_ptr->stp_p1_base = 0x400000;
    bk_ptr->stp_p0_base = 0;
...
// Based on Michael Chen's  (micchen@altera.com) implementation 
